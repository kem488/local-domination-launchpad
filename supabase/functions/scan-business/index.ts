  ...
  try {
    ...
    try {
      ...
      if (recommendationsResponse.ok) {
        ...
        // Generate and save fallback recommendations immediately
        const fallbackRecommendations = generateFallbackRecommendations(scores, placeDetails);
        await supabase
          .from('business_scans')
          .update({ 
            ai_recommendations: JSON.stringify(fallbackRecommendations)
          })
          .eq('id', scanData.id);
      }
    } catch (error) {
...
function generateFallbackRecommendations(scores: any, place: GooglePlaceDetails) {
  const quickWins = [];
